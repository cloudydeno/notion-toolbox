...
import * as XmlEntities from "@std/html/entities";
...
export async function fetchPhoto(instagramUrl: string) {
  ...
  const caption = XmlEntities.unescape(linkingData.caption) as string;
...
function preview_to_jpeg_url(inputString: string) {
	const dynamic = decodeBase64(inputString);
	const payload = dynamic.subarray(3);
	const template = decodeBase64(jpegtpl);
	template[162] = dynamic[1];
  template[160] = dynamic[2];
  const final = new Uint8Array(template.length + payload.length);
  final.set(template);
  final.set(payload, template.length);
  return 'data:image/jpg;base64,'+encodeBase64(final);
};
